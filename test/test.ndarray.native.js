...
var scuevery = tryRequire( resolve( __dirname, './../lib/ndarray.native.js' ) );
var opts = {
...
tape( 'the function has an arity of 7', opts, function test( t ) {
	t.strictEqual( scuevery.length, 7, 'has expected arity' );
	t.end();
});
...
tape( 'the function returns a reference to the output array', opts, function test( t ) {
	...
	out = scuevery( x.length, x, 1, 0, y, 1, 0 );
...
tape( 'the function cumulatively tests whether every element is truthy', opts, function test( t ) {
	...
	scuevery( x.length, x, 1, 0, out, 1, 0 );
	t.ok( isEqualBooleanArray( out, expected ), 'returns expected value' );
...
tape( 'the function supports offsets for `x`', opts, function test( t ) {
	...
	x = new Float32Array( [ 999.0, 1.0, 1.0, 0.0, 0.0 ] );
	out = new BooleanArray( 4 );
	expected = new BooleanArray( [ true, true, false, false ] );

	scuevery( 4, x, 1, 1, out, 1, 0 );
	t.ok( isEqualBooleanArray( out, expected ), 'returns expected value' );
...
tape( 'the function treats `NaN` as a falsy element', opts, function test( t ) {
	...
	x = new Float32Array( [ 1.0, NaN, 1.0, 1.0 ] );
	out = new BooleanArray( 4 );
